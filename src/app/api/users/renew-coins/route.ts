...
if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.cert({
      ...
      clientEmail: "firebase-adminsdk-6ak35@gameroom-416719.iam.gserviceaccount.com",
...
async function sendNotification(user: any) {
  const notificationData = {
    email: user.email,
    message: "Suas fichas foram renovadas, volte agora a jogar para conquistar o seu prêmio",
  ...
  try {
    await request("/notifications", "POST", notificationData, undefined, true, undefined, headers);
...
async function handlePostRequest(email: string) {
  ...
  if (!email) {
    return NextResponse.json({ message: "Invalid e-mail to eligible for renewal." });
  }
